...
void loop() {
  bool buttonStateUp = digitalRead(BUTTON_PIN1);
  bool buttonStateDown = digitalRead(BUTTON_PIN2);

  // Detect a button press (falling edge)
  if (lastButtonState1 == LOW && buttonStateUp == HIGH) {
    mutateNumber(true);
  }
  if (lastButtonState2 == LOW && buttonStateDown == HIGH) {
    mutateNumber(false);
  }

  lastButtonState1 = buttonStateUp;
  lastButtonState2 = buttonStateDown;
  printf((char*)lastButtonState1);
...
void testanimate(const uint8_t *bitmap, uint8_t w, uint8_t h) {
  ...
  for(int i = 0; i < 122; i++) { // Loop forever...
    ...
    delay(1000);        // Pause for 1/10 second

    display.oled_command(0x81); // or directly 0xD9
    if(i % 2) {
      // Screen all the way dimmed
      display.oled_command(0); // value between 0–255
    } else {
      // Screen all the way bright
      display.oled_command(255); // value between 0–255
    }
